...
| Dart bridge | Официальный binary release 1.8.0. Проверяются SHA-256 исходного файла и совпадение ELF-секций |
...
сборки отсутствует в предоставленном артефакте, поэтому для теста его чтения
использована явно помеченная фикстура. Подлинный resolved lock новой сборки
обязательно сохранит workflow. Локальный результат не подтверждает эту ещё
не выполненную CI-проверку.
...
После коммита этого изменения нужен новый запуск `build-linux.yml` и матрица
`test-linux-appimage.yml` с ID новой сборки и её полным commit SHA. Прежние
байты `4a498ad` остаются диагностическим артефактом без внесённых notices.
